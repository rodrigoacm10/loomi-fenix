          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {analysis.insights.insights.map((insight) => (
                <div
                  key={insight.id}
                  className="bg-[#242b3d] p-3 rounded-xl border border-white/5"
                >
                  <div className="flex items-center gap-2 mb-1">
                    <Activity className="w-4 h-4 text-[#4dd4ce]" />
                    <span className="text-xs text-[#4dd4ce] font-medium">
                      {insight.type}
                    </span>
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {analysis.futureSteps.actions.map((step) => (
                <div
                  key={step.id}
                  className="flex items-start gap-3 bg-[#242b3d] p-3 rounded-xl border border-white/5"
                >
                  <CheckCircle2
                    className={`w-5 h-5 mt-0.5 shrink-0 ${step.priority === 'high' ? 'text-loomi-danger' : step.priority === 'medium' ? 'text-orange-400' : 'text-loomi-secondary'}`}
                  />
                  <div className="flex-1">
                    <p className="text-sm text-white">{step.action}</p>
                    <p className="text-[10px] text-loomi-muted mt-1 uppercase">
