...
export function ClientDataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      ...
      const email =
        (row.original as { email?: string }).email?.toLowerCase() || ''
    ...
    <div className="w-full max-w-full">
      <div className="flex flex-col md:flex-row items-center pt-2 pb-4 gap-2 lg:flex-nowrap flex-wrap w-full">
        ...
        <div className="flex gap-2 w-full">
          ...
          <Select
            value={
              (table.getColumn('secureType')?.getFilterValue() as string) ??
              'Todos'
            }
            onValueChange={(value) =>
              table
                .getColumn('secureType')
                ?.setFilterValue(value === 'Todos' ? undefined : value)
            }
          >
      ...
      <div className="rounded-4xl bg-loomi-table px-6 pb-2 lg:w-full max-w-full overflow-x-auto">
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow
                className="border-white/10 hover:bg-transparent text-sm"
                key={headerGroup.id}
              >
