...
export function DashboardMap() {
  ...
  const types = useMemo(
    () =>
      Array.from(new Set(mapLocations.map((l) => l.category)))
        .filter(Boolean)
        .sort(),
    [mapLocations],
  )
    ...
    return filteredLocations
      .map((location) => {
        ...
        const iconUrl =
          ...
          encodeURIComponent(svgString || '<svg></svg>')
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Select value={selectedLocal} onValueChange={setSelectedLocal}>
            <SelectTrigger className="w-full sm:w-45 bg-loomi-bg-card border-none text-white h-9 rounded-full px-4">
              <SelectValue
          ...
          <Select value={selectedType} onValueChange={setSelectedType}>
            ...
            <SelectContent className="bg-loomi-bg-card border-[#2e344d] text-white">
              ...
              {types.map((type) => (
                <SelectItem
                  className="focus:bg-loomi-primary focus:text-white capitalize"
                  key={type}
                  value={type}
                >
                  {type}
