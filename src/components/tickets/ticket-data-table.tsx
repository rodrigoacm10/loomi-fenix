...
export function TicketDataTable<TData, TValue>({
  ...
  const currentResponsibleFilter =
    ...
    <div className="w-full max-w-full">
      <div className="flex flex-col md:flex-row items-center pt-2 pb-4 gap-2 lg:flex-nowrap flex-wrap w-full">
        ...
        <div className="flex gap-2 w-full">
          ...
          >
            <SelectTrigger className="flex-1 lg:w-45 px-5 bg-loomi-bg-dark border-0 rounded-full h-9">
          ...
          <Popover open={isResponsibleOpen} onOpenChange={setIsResponsibleOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                className="overflow-hidden flex-1 lg:w-50 xl:w-[320px] px-5 bg-loomi-bg-dark hover:bg-loomi-bg-dark hover:text-white border-0 rounded-full justify-between h-9"
              >
                {currentResponsibleFilter
                  ? uniqueResponsibles.find(
                      (responsible) => responsible === currentResponsibleFilter,
                    )
                  : t('allResponsibles')}
                <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-50 p-0 border-0 bg-loomi-bg-dark">
              <Command
                shouldFilter={false}
                className="bg-loomi-bg-dark border-0 text-white"
              >
                <CommandInput
                        ...
                        {uniqueResponsibles
                          ...
                          .map((responsible) => (
                            <div
                              ...
                              onClick={() => {
                                table
                                  .getColumn('responsible')
                                  ?.setFilterValue(
                                    responsible === currentResponsibleFilter
                                      ? undefined
                                      : responsible,
                                  )
                                setIsResponsibleOpen(false)
                            ...
                            >
                              <Check
                                className={cn(
                                  'mr-2 h-4 w-4',
                                  currentResponsibleFilter === responsible
                                    ? 'opacity-100'
                                    : 'opacity-0',
                                )}
                              />
      ...
      <div className="rounded-4xl bg-loomi-table px-6 pb-6 lg:w-full max-w-full overflow-x-auto">
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow
                className="border-white/10 hover:bg-transparent text-sm !py-2"
                key={headerGroup.id}
              >
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      className="px-0 !pr-8 py-4 text-loomi-muted"
                      key={header.id}
                    >
