...
export function TicketDialog() {
    ...
    return (
        <Dialog open={isTicketModalOpen} onOpenChange={(open) => !open && closeTicketModal()}>
            <DialogContent showCloseButton={false} className="!rounded-[24px] sm:max-w-[500px] text-white bg-loomi-bg-dark border-0">
                <DialogHeader className="space-y-4">
                    <div className="flex items-center justify-between pointer-events-none">
                        <DialogTitle className="text-xl font-normal pointer-events-auto">
                            {isViewMode ? t("viewTicket") : (selectedTicket ? t("editTicket") : t("newTicket"))}
                        </DialogTitle>
