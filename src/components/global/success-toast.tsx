...
export function SuccessToast({ t, title, description }: SuccessToastProps) {
  return (
    <div className="flex items-start space-x-4 rounded-lg bg-loomi-primary p-4 py-5 text-white shadow-lg pointer-events-auto w-112.5">
      <div className="-mt-1 shrink-0 bg-white rounded-full p-1">
        <Check color="#1876D2" className="h-4 w-4 text-white" strokeWidth={3} />
      </div>
      <div className="flex-1 space-y-1">
        <p className="font-bold text-[16px] leading-none text-white">{title}</p>
        <p className="text-[14px] text-white/90">{description}</p>
      </div>
      <button
