export function createCustomTooltip({
  ...
  switch (activeMetric) {
    case 'conversion':
      metricNameStr = 'novos clientes'
      break
    case 'arpu':
      metricNameStr = ''
      break
    case 'retention':
      metricNameStr = 'retidos'
      break
    case 'churn':
      metricNameStr = 'perdidos'
      break
    default:
      metricNameStr = activeMetric
      break
  }

  return `
        <div class="relative flex items-center h-10 px-4 ${isRightSide ? 'mr-2' : 'ml-2'} bg-[#383d4e] rounded-lg shadow-lg">
            <div class="absolute ${isRightSide ? '-right-1.5' : '-left-1.5'} top-1/2 -translate-y-1/2 w-0 h-0 border-y-[6px] border-y-transparent ${isRightSide ? 'border-l-[6px] border-l-[#383d4e]' : 'border-r-[6px] border-r-[#383d4e]'}"></div>
